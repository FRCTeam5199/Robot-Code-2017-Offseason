...
public class Robot extends SampleRobot {
	...
	public static long climbTime = 0;
	...
	public void autonomous() {
		...
		while (isAutonomous() && isEnabled()) {
			...
			if (autoMode.equals("e") || autoMode.equals("E")) {
				...
				if (autoStep == 4) {
					if (driveEncoders.driveStraightAuton(40)) {
						autoStep = 5;
					}
				}
				if (autoStep == 5) {
				...
				if (autoStep == 6) {
					if (ultraFunctions.driveFowardAutonSDR(4)) {
						autoStep = 7;
					}
				}
				if (autoStep == 7) {
				...
				if (autoStep == 3) {
					...
					if (ultraFunctions.driveFowardAutonSDR(6)) {
						autoStep = 4;
					}
					// }
				}
				...
				if (autoStep == 1) {
					...
					// if (driveEncoders.driveStraightAuton(38)) {
					if (driveEncoders.driveStraightAuton(90)) {
						autoStep = 2;
					}
				}
				if (autoStep == 2) {
	...
	public void operatorControl() {
		...
		while (isOperatorControl() && isEnabled()) {
			...
			if (xBox.getRawButton(1)) {
				...
				if (pixyFunctionsGear.turnAndGoStraightAuton() && ultraData.distanceLeft() < 8) {

					ultraFunctions.driveFowardAutonSDR(4);
				}
			}
