...
public class Robot extends SampleRobot {
	...
	public void operatorControl() {
		...
		double driveMod = 1;
		double rightBoost = 0, leftBoost = 0;
		...
		while (isOperatorControl() && isEnabled()) {
			...
			if (xBox.getRawButton(1)) {
				...
				step = 0;
				// Driver.drive(xBox.getRawAxis(1), xBox.getRawAxis(5), 1);
				if(xBox.getRawAxis(1) > 0){
					leftBoost = (xBox.getRawAxis(3)* .25);
				}else{
					leftBoost = (xBox.getRawAxis(3) * -.25);
				}
				
				if(xBox.getRawAxis(5) > 0){
					rightBoost = (xBox.getRawAxis(3)* .25);
				}else{
					rightBoost = (xBox.getRawAxis(3) * -.25);
				}
				
				right.set((xBox.getRawAxis(5) * driveMod) + rightBoost);
				left.set((-xBox.getRawAxis(1) * driveMod) - leftBoost);
			}
