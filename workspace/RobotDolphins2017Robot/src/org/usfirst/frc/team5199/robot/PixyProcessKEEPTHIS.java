...
public class PixyProcessKEEPTHIS {
	private static final int debugLevel = 1; // edit depending on future needs
	public static Pixy pixyCam, pixyGear,pixyShooter;
	...
	public PixyProcessKEEPTHIS(Pixy pixy) {
...
//		SmartDashboard.putNumber("Distance off", averageData(0, false)[0] - 160);
	...
	public static void pixyI2CTest() {
		...
		if (pixyCam.getByte() > 0) {
			gearByteCount++;
			SmartDashboard.putNumber("I2C Test Byte Count Gear", gearByteCount);
			SmartDashboard.putString("I2C Test Status Gear" , "I2C  receiving data");
			SmartDashboard.putString("Actually worked", "YES YES YES");
		}
	...
	public static double[] averageData(int mode, boolean displayResults) {
	...
	public static double[] shooterData() {
		if (pixyCam.getStartOfData() == 1) {
			...
			syncedBufferWithoutSync = pixyCam.getVariableSizeBuffer(14);
			SyncedLongBlock block = new SyncedLongBlock(syncedBufferWithoutSync);
