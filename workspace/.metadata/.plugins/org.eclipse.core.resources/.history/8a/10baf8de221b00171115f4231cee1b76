	...
	public void autonomous() {
		...
		while (isAutonomous() && isEnabled()) {
			...
			if (!fuckthisshit) {
				...
				if (autoStep == 0) {
					if (driveFunc.driveStraightAuton(73)) {
						autoStep = 1;
					}
				}
				if (autoStep == 1) {
				...
				if (autoStep == 2) {
					if (bank.ultraDistanceLeft() > 12 && bank.ultraDistanceRight() > 12) {
						if (pixyGearFunc.turnAndGoStraightAuton()) {
							Driver.drive(0.25, 0, 1);
						}
					} else {
						if (ultraFunctions.driveFowardAuton(5)) {
							autoStep = 3;
						}
					}
				...
				if (autoStep == 1) {
					if (bank.ultraDistanceLeft() > 16 && bank.ultraDistanceRight() > 16) {
						...
						if (ultraFunctions.driveFowardAuton(8)) {
							autoStep = 2;
						}
					}
				}
