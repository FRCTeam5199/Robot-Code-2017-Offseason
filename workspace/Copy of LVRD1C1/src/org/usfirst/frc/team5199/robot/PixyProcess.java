...
public class PixyProcess {
	...
	public void pixyGearTest() {
		...
		SmartDashboard.putNumber("Gear PixyBlocks per second", blocksPerSecond());
		// SmartDashboard.putNumber("Gear Avg X", averageData(0, false,
		// gearPixy)[0] );
		// SmartDashboard.putNumber("Gear Distance off", averageData(0,
		// false,gearPixy)[0] - 160);
	...
	public double[] shooterAverageData(int mode, boolean displayResults) {
		...
		if (shooterPixy.getStartOfData() == 1) {
																							...
																							// 0xaa
			{
				...
				if (mode == 2) {
					shooterSumOfBufferBetweenX += (Math.abs(block.getX(0) - block.getX(1)))
							- shooterAverageDataValueArrayBetweenX[counter];
				}
	...
	private static double blocksPerSecond() {
		...
		if ((currentTime - lastTime) > 1000) {
			gearBlockCount = tempBlockCount;
			tempBlockCount = 0;
			lastTime = currentTime;
		}
		return gearBlockCount;
